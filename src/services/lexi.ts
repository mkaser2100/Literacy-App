...
export function startSession():Session {
  ...
  write(SESSIONS_KEY,[...read<Session>(SESSIONS_KEY),session]); return session;
}
export function saveAttempt(attempt:Attempt){write(ATTEMPTS_KEY,[...read<Attempt>(ATTEMPTS_KEY),attempt]);}
export function finishSession(id:string,summary:Omit<Session,'id'|'startedAt'>){
  write(SESSIONS_KEY,read<Session>(SESSIONS_KEY).map(s=>s.id===id?{...s,...summary}:s));
}
...
export function speak(text:string){
  ...
  speechSynthesis.cancel(); const u=new SpeechSynthesisUtterance(text); u.rate=.78; speechSynthesis.speak(u);
}
