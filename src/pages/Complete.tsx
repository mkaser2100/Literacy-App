...
import { getLastSummary } from '../services/lexi';

export default function Complete({home}:{home:()=>void}) {
  const s=getLastSummary()||{durationMs:0,accuracy:0,stars:0,xp:38};
  const mins=Math.floor(s.durationMs/60000);
  const secs=Math.floor((s.durationMs%60000)/1000).toString().padStart(2,'0');
  return <main className="complete-screen">
    ...
    <div className="xp-pill">+{s.xp} XP</div>
    <section className="complete-stats">
      ...
      <div><span className="complete-stat-icon purple"><Star/></span><strong>{s.accuracy}%</strong><small>First try</small></div>
      <div><span className="complete-stat-icon blue"><Timer/></span><strong>{mins}:{secs}</strong><small>Time</small></div>
    </section>
    <section className="streak-panel"><Flame size={35} fill="currentColor"/><div><strong>Practice saved!</strong><span>Your lesson history is safely stored.</span></div></section>
